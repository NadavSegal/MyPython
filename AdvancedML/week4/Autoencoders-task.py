...
import keras_utils2 # by NS
#tf.disable_eager_execution() # by NS
tf.enable_eager_execution() # by NS
...
autoencoder.fit(x=X_train, y=X_train, epochs=15,
                ...
                callbacks=[keras_utils2.TqdmProgressCallback()
                ],
                verbose=0)
...
def build_deep_autoencoder(img_shape, code_size):
    ...
    #encoder.add(L.MaxPooling2D(pool_size=(3, 3), strides=None, padding='same'))
    ...
    decoder.add(L.Dense(int(H/16*W/16*256))) 
    decoder.add(L.Reshape((int(H/16),int(W/16),256)))
...
autoencoder.fit(x=X_train, y=X_train, epochs=25,
                validation_data=[X_test, X_test],
                callbacks=[keras_utils2.ModelSaveCallback(model_filename),
                           keras_utils2.TqdmProgressCallback()],
                verbose=0,
...
COURSERA_TOKEN = 'pGsgmpXFlOUBccDA'
